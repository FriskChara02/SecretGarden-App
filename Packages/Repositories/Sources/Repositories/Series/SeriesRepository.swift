...
public final class SeriesRepository: SeriesRepositoryProtocol {
    ...
    public func fetchChapterPages(chapterId: String) async throws -> [ChapterPage] {
        try await apiClient.request(SeriesEndpoint.chapterPages(chapterId: chapterId))
    }

    public func recordReadingProgress(seriesId: String, chapterId: String, page: Int) async throws {
        try await apiClient.requestWithoutResponse(
            SeriesEndpoint.recordReadingProgress(
                seriesId: seriesId,
                chapterId: chapterId,
                request: RecordReadingProgressRequest(page: page)
            )
        )
    }
    ...
    public func updateReadingStatus(seriesId: String, status: ReadingStatus, notifyNewChapter: Bool) async throws {
        try await apiClient.requestWithoutResponse(
            SeriesEndpoint.updateReadingStatus(
                seriesId: seriesId,
                request: UpdateReadingStatusRequest(status: status, notifyNewChapter: notifyNewChapter)
            )
        )
    }
    ...
    public func toggleNotify(seriesId: String, enabled: Bool) async throws {
        try await apiClient.requestWithoutResponse(
            SeriesEndpoint.toggleNotify(seriesId: seriesId, request: ToggleNotifyRequest(enabled: enabled))
        )
    }
